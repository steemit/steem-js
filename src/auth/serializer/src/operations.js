...
const transfer_to_vesting = new Serializer("transfer_to_vesting", {
  from: string,
  to: string,
  amount: asset
});

const withdraw_vesting = new Serializer("withdraw_vesting", {
  account: string,
...
const feed_publish = new Serializer("feed_publish", {
  publisher: string,
  exchange_rate: price
});

const convert = new Serializer("convert", {
  owner: string,
  requestid: uint32,
  amount: asset
});
...
const report_over_production = new Serializer("report_over_production", {
  reporter: string,
...
const custom_json = new Serializer("custom_json", {
  required_auths: set(string),
...
const limit_order_create2 = new Serializer("limit_order_create2", {
  owner: string,
  orderid: uint32,
  amount_to_sell: asset,
  exchange_rate: price,
...
const claim_account = new Serializer("claim_account", {
  creator: string,
  fee: asset,
  extensions: set(future_extensions)
});

const create_claimed_account = new Serializer("create_claimed_account", {
  creator: string,
  new_account_name: string,
  owner: authority,
  active: authority,
...
const pow2_input = new Serializer("pow2_input", {
  worker_account: string,
  prev_block: bytes(20),
  nonce: uint64
});

const pow2 = new Serializer("pow2", {
  input: pow2_input,
  pow_summary: uint32
});

const equihash_proof = new Serializer("equihash_proof", {
  n: uint32,
  k: uint32,
  seed: bytes(32),
  inputs: array(uint32)
...
const equihash_pow = new Serializer("equihash_pow", {
  input: pow2_input,
...
const transfer_to_savings = new Serializer("transfer_to_savings", {
  from: string,
  to: string,
  amount: asset,
  memo: string
...
const transfer_from_savings = new Serializer("transfer_from_savings", {
  from: string,
  request_id: uint32,
  to: string,
  amount: asset,
...
const cancel_transfer_from_savings = new Serializer(
...
const custom_binary = new Serializer("custom_binary", {
...
const reset_account = new Serializer("reset_account", {
  reset_account: string,
  account_to_reset: string,
  new_owner_authority: authority
});

const set_reset_account = new Serializer("set_reset_account", {
  account: string,
  current_reset_account: string,
  reset_account: string
});

const claim_reward_balance = new Serializer("claim_reward_balance", {
  account: string,
  reward_steem: asset,
  reward_sbd: asset,
  reward_vests: asset
...
const delegate_vesting_shares = new Serializer("delegate_vesting_shares", {
  delegator: string,
  delegatee: string,
  vesting_shares: asset
});

const account_create_with_delegation = new Serializer(
...
const witness_set_properties = new Serializer("witness_set_properties", {
  owner: string,
  props: string,
  extensions: set(future_extensions)
});

const account_update2 = new Serializer("account_update2", {
  account: string,
  owner: optional(authority),
  active: optional(authority),
  posting: optional(authority),
...
const update_proposal_votes = new Serializer("update_proposal_votes", {
  voter: string,
...
const remove_proposal = new Serializer("remove_proposal", {
  proposal_owner: string,
  proposal_ids: array(uint64),
  extensions: set(future_extensions)
});

const fill_convert_request = new Serializer("fill_convert_request", {
  owner: string,
  requestid: uint32,
  amount_in: asset,
  amount_out: asset
...
const liquidity_reward = new Serializer("liquidity_reward", {
  owner: string,
  payout: asset
});

const interest = new Serializer("interest", {
  owner: string,
  interest: asset
});

const fill_vesting_withdraw = new Serializer("fill_vesting_withdraw", {
  from_account: string,
  to_account: string,
  withdrawn: asset,
  deposited: asset
...
const fill_transfer_from_savings = new Serializer(
...
const return_vesting_delegation = new Serializer("return_vesting_delegation", {
  account: string,
  vesting_shares: asset
});
